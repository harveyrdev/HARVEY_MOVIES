import type {
  ...
  export const getRandom = <T>({ collections }: GetRandomMedia<T>) => {
    const items = collections.flatMap((collection) => collection.results || []);
    const randomItem = items.sort(() => Math.random() - 0.5);
    const selectedItems = randomItem.slice(0, 5);
  ...
  export const getTrendingMovieRR = ({ page }: GetTrendingMovie) => {


    const data= fetchTMDB<Collection<MovieMedia>>(`trending/movie/week`, {
      page: String(page),
    });
